...
- **Verbatim engine, verbatim tests.** `core.js` is the upstream `impl`
  ...
  *is* the page), wrapped in an IIFE so that re-injecting it is idempotent
  (see *Fork* above). Its body is otherwise untouched and deliberately not
  re-indented, so it still diffs cleanly against the original. All 188
  upstream self-test cases run unmodified.
...
tools/             gen-icons.sh, build.mjs, gen-patterns-doc.mjs, store-assets.mjs
docs/              pattern reference, privacy policy, publishing guide
store/             generated listing images (npm run assets)
